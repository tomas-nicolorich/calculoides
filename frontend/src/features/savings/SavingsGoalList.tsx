...
export function SavingsGoalList({ goals, onRefresh }: SavingsGoalListProps) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-bold tracking-tight text-slate-800 dark:text-slate-200">Current Goals</h2>
        ...
        {goals.map((goal) => {
          ...
          return (
            <Card key={goal.id} className={cn(
              ...
              isSuccess ? "ring-2 ring-emerald-500 bg-emerald-50/30 border-l-emerald-500" : "border-l-brand-balance",
              isLate && !isSuccess ? "border-l-brand-expense" : ""
            )}>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <span className="text-slate-400 hover:text-brand-balance transition-colors">✎</span>
                      </Button>
                    </div>
                  ...
                  <div className="text-right">
                    <p className={cn(
                      ...
                      isLate ? 'bg-brand-expense/10 text-brand-expense border border-brand-expense/20' : 'bg-emerald-100 text-emerald-700 border border-emerald-200'
                    )}>
                      {isLate ? `Delayed ${goal.varianceMonths.toString()}mo` : 'On Track'}
                    </p>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-xs bg-slate-50 dark:bg-slate-900/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
                    <div>
                      <p className="text-slate-400 font-medium mb-0.5">Target Date</p>
                      <p className="font-bold text-slate-700 dark:text-slate-300">{targetDate.toLocaleDateString()}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-slate-400 font-medium mb-0.5">Projected</p>
                      <p className={`font-bold ${isLate ? 'text-brand-expense' : 'text-emerald-600'}`}>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-1">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Monthly Allocation</p>
                      <Button 
                        variant="ghost" 
                        size="sm" 
                        className="h-5 px-2 text-[9px] font-bold text-brand-balance hover:bg-brand-balance/10"
                    ...
                    {goal.breakdown.map((item) => (
                      <div key={item.memberId} className="flex justify-between items-center py-1">
                        <UserDisplay user={item.user} className="font-medium text-slate-700 dark:text-slate-300" />
                        <div className="flex items-center gap-2">
                          {adjustingGoalId === goal.id ? (
                            ...
                            <div className="text-right">
                              <p className="font-bold text-slate-900 dark:text-white">€{item.actualAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                              {item.isOverridden && (
                                <p className="text-[9px] font-bold text-brand-balance bg-brand-balance/5 px-1.5 rounded-full inline-block">CUSTOM</p>
                              )}
                            </div>
                          )}
                        </div>
