...
import { CategoryWithBalances, CategoryBalance } from '../../../../../shared/src/types/redesign';
...
export function BudgetCategories({ 
  ...
  // Transfer State
  const [transferCategory, setTransferCategory] = useState<{id: string, name: string} | null>(null);
  ...
  const isFormDirty = name !== '' || monthlyBudget !== '' || selectedMemberIds.length > 0;
  const isTransferDirty = transferAmount !== '';
  ...
  const handleAddSubmit = async (e: React.SyntheticEvent) => {
    ...
    setFormError(null);
    try {
      await apiClient.fetch(`/transactions?action=category-create&groupId=${groupId}`, {
        method: 'POST',
        body: JSON.stringify({
          name,
  ...
  const handleTransferSubmit = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      setFormError(err instanceof Error ? err.message : 'Failed to transfer budget');
    } finally {
      setFormLoading(false);
    }
  };
    ...
    >
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div className="text-sm text-slate-500">Categories and per-member breakdown</div>
          <button 
        ...
        >
          <form onSubmit={(e) => void handleAddSubmit(e)} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500">If none selected, category applies to everyone.</p>
            </div>
            {formError && <p className="text-sm text-red-500">{formError}</p>}
            <button type="submit" disabled={formLoading} className="w-full p-3 mt-4 bg-brand-balance text-white rounded-xl font-medium disabled:opacity-50">
        ...
        >
          <form onSubmit={(e) => void handleTransferSubmit(e)} className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">From</label>
              <select
                className="w-full p-2 rounded-lg border border-slate-200 dark:border-slate-700 dark:bg-slate-800"
                value={transferFromMemberId}
                onChange={(e) => { setTransferFromMemberId(e.target.value); }}
                disabled={!isOwner}
                required
              >
                {members.map((m) => (
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {members.map((m) => (
                  <option key={m.id} value={m.id} disabled={m.id === transferFromMemberId}>{m.name}</option>
                ))}
              </select>
            </div>
            <div className="space-y-2">
            ...
            {formError && <p className="text-sm text-red-500">{formError}</p>}
            <button type="submit" disabled={formLoading || !transferToMemberId} className="w-full p-3 mt-4 bg-brand-transfer text-white rounded-xl font-medium disabled:opacity-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {categories.map((category) => (
            <div key={category.id} className="p-4 rounded-2xl border border-slate-100 dark:border-slate-800 space-y-4">
              <div className="flex justify-between items-start">
                <div>
                  <h4 className="font-bold text-slate-900 dark:text-white text-lg">{category.icon} {category.name}</h4>
                  <p className="text-sm text-slate-500">Target: {formatCurrency(category.monthlyBudget)}</p>
                </div>
              ...
              <div className="space-y-3">
                {category.balances.map((balance: CategoryBalance) => (
                  <div key={balance.memberId} className="flex justify-between items-center text-sm p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                    ...
                    <div className="text-right">
                      <div className="font-medium text-slate-900 dark:text-white">
                        {formatCurrency(balance.remainingQuota)} <span className="text-xs text-slate-400 font-normal">left</span>
                      </div>
                      <div className="text-[10px] text-slate-500">
                        Budget: {formatCurrency(balance.quota)} | Spent: {formatCurrency(balance.spent)}
                      </div>
                    </div>
                  </div>
                ))}
