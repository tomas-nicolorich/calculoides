...
- [ ] T022b [P] In `frontend/tests/pages/dashboard.test.tsx` verify the grid container has class `items-start` and does not have `self-stretch` or `h-full` on any card element — guard against the most common regression path for SC-DS-007 (sibling height-stretching). Run `npm test -w frontend` — must pass before marking Phase 6 complete.
