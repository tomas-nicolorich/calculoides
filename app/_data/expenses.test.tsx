...
import { useExpensesList, useCreateExpense, useUpdateExpense } from "./expenses";

vi.mock("../../lib/actions/expense", () => ({
  create: vi.fn().mockResolvedValue({ ok: true, data: { id: "expense-1" } }),
  update: vi.fn().mockResolvedValue({ ok: true, data: { id: "expense-1" } }),
  deleteExpense: vi.fn(),
  deleteAll: vi.fn(),
}));
...
describe("app/_data/expenses", () => {
  ...
  it("useUpdateExpense invalidates the group's cache on success", async () => {
    const queryClient = createQueryClient();
    await queryClient.prefetchQuery({
      queryKey: queryKeys.summary(GROUP_ID),
      queryFn: () => Promise.resolve({ groupName: "Roomies" }),
    ...
    const { result } = renderHook(() => useUpdateExpense(GROUP_ID), {
      wrapper,
    });
