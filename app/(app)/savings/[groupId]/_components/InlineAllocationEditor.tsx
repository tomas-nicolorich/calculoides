...
import { Avatar, Badge, Button, Input, UserDisplay } from "../../../../_ui";
...
} from "../../../../_data/useContributionSession";
...
interface AllocationRowProps {
  item: SavingsContributionBreakdown;
  index: number;
  isEditing: boolean;
  loading: boolean;
  session: ContributionSession;
...
function AllocationAmountDisplay({
  item,
}: {
  item: SavingsContributionBreakdown;
}) {
  return (
    <div className="text-right flex items-center gap-2 shrink-0">
      ...
      <p className="font-semibold text-slate-900 dark:text-white font-mono tabular-nums">
...
/**
 * Full port of prod's `frontend/src/features/savings/InlineAllocationEditor.tsx`.
 * Owns its own `useContributionSession` (`app/_data/savings.ts`) and saves
 * via `session.saveSession()` — the single canonical persistence path for
 * contribution overrides. Always renders the member list; a single Adjust
 * toggle swaps every row's amount for an input at once instead of a
      ...
      {isEditing && (
        <div className="space-y-2 transition-all duration-200 ease-out starting:opacity-0 starting:-translate-y-1">
          ...
          <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">
            ...
            <span
              className={`font-mono tabular-nums font-semibold ${FORECAST_COLOR_CLASS[session.forecastColor]}`}
            >
