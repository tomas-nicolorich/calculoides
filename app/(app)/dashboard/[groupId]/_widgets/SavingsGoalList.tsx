...
import { DialogFooter } from "../../../../_ui/Dialog";
import { ResponsiveDialog } from "../../../../_ui/ResponsiveDialog";
import { formatCurrency } from "../../../../../lib/format-currency";
import { cn } from "../../../../../lib/cn";
import { useSavingsGoalsList, useDeleteGoal } from "../../../../_data/savings";
...
/**
 * Ported from `main`'s `frontend/src/features/savings/SavingsGoalList.tsx`
 * (PR 16, tasks 16.3-16.5). Data seam: self-subscribes to the hydrated
 * `queryKeys.savingsGoals` cache (`page.tsx` prefetches it, 16.1-16.2) —
 * same self-subscribing convention `BudgetCategories`/`RecentExpenses`
 * established, own loading/error state independent of the other five
 * widgets (spec: "Loading state precedes hydration"). Adds placement only
 * — goal/contribution math stays owned by `savings-goal-management`/
 * `savings-income-split-allocation` (`SavingsGoalForm`, `useContributionSession`).
 */
export function SavingsGoalList({ groupId }: { groupId: string }) {
  const { data: goals, isLoading, isError } = useSavingsGoalsList(groupId);
  const deleteGoal = useDeleteGoal(groupId);
  ...
  if (isLoading) {
    return (
      <Card title="Savings Goals" data-testid="savings-goal-list-loading">
  ...
  const deleteError =
    deleteGoal.error instanceof Error ? deleteGoal.error.message : null;

  const goalToDelete = goals.find((g) => g.id === goalToDeleteId) ?? null;

  const handleDelete = async (goal: SavingsGoal) => {
    const result = await deleteGoal.mutateAsync({ goalId: goal.id });
    if (result.ok) setGoalToDeleteId(null);
  };
