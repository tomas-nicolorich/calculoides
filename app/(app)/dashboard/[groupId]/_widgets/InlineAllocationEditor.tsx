      ...
      <Popover.Portal>
        ...
        >
          <Popover.Popup className="w-64 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-3 text-xs text-slate-600 dark:text-slate-300 animate-in fade-in-50 zoom-in-95 duration-100">
            This member&apos;s share exceeds what&apos;s currently available
            in the shared balance. You can still save, but the group balance
...
/**
 * Ported from `main`'s `frontend/src/features/savings/InlineAllocationEditor.tsx`
 * (PR 16). Owns its own `useContributionSession` and saves via
 * `session.saveSession()` — the single canonical persistence path for
