...
interface SelectProps {
  ...
  id?: string;
  /**
   * id of an external `<label>`. Combined with the trigger's own value node
   * via aria-labelledby so the accessible name is "<label> <selected value>"
   * instead of the label text alone overriding the selection.
   */
  labelId?: string;
}

/** Ported verbatim from `main`'s `shared/ui/Select.tsx`. */
export function Select({
  ...
  return (
    ...
    >
      ...
      >
        <BaseSelect.Value id={valueId} placeholder={placeholder}>
          {(val: string | null) =>
