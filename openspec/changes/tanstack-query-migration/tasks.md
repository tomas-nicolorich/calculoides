...
## Status: COMPLETE — 48/48 tasks across 5 PRs (S1, 2a, 2b, 3a, 3b)

All five chained PRs on the `tanstack-query-migration` feature-branch chain are implemented and green: `caf4989` (PR1), `fa14e7f` (PR2), `5dfa56d` (PR3), `0a00d3f` (PR4), and this batch's PR5 commit. Every hand-threaded `refresh`/`onRefresh` mutation-adjacent closure has been replaced by `useMutation` + `queryClient.invalidateQueries`. Remaining `refresh()` call sites repo-wide are exclusively the standard `ApiQueryResult.refresh()` retry-button affordance (D7), not mutation-refresh wiring.
...
## Slice 3b (PR5): Remaining Page/Feature Mutations — COMPLETE (17/17)

- [x] 3b.1 RED — extended `frontend/src/features/savings/SavingsGoalForm.test.tsx`: create/update invalidate `queryKeys.group(groupId)` (D6 default prefix, matching PR4's precedent — see Deviations), no refresh callback prop asserted.
- [x] 3b.2 GREEN — modified `frontend/src/features/savings/SavingsGoalForm.tsx`: `useMutation` + `savingsGoalApi.create`/`update` + `invalidateQueries`; `onSuccess` prop kept (dialog-close signal only, no longer refresh-triggering).
- [x] 3b.3 RED — extended `frontend/src/features/savings/SavingsGoalList.test.tsx`: goal delete invalidates `queryKeys.group(groupId)`; dropped `onRefresh` prop and its assertions; rewrote the dialog-stays-open-until-refresh-resolves test to control `client.invalidateQueries` timing instead of an `onRefresh` promise.
- [x] 3b.4 GREEN — modified `frontend/src/features/savings/SavingsGoalList.tsx`: `useMutation` for delete + invalidation; dropped `onRefresh` prop entirely (including from `InlineAllocationEditor`/`SavingsGoalForm` call sites).
- [x] 3b.5 RED — extended `frontend/src/entities/savings-goal/useContributionSession.test.ts`: added `QueryWrapper` to every `renderHook` call (new requirement — the hook now calls `useQueryClient()` unconditionally).
- [x] 3b.6 GREEN — modified `frontend/src/entities/savings-goal/useContributionSession.ts` + `frontend/src/features/savings/InlineAllocationEditor.tsx`: `useMutation` wrapping `savingsGoalApi.upsertContribution`/`deleteContribution`, `onSuccess` invalidates `queryKeys.group(activeGoal.groupId)`; dropped `InlineAllocationEditor`'s `onRefresh` prop entirely.
- [x] 3b.7 RED — extended `frontend/src/entities/member/useIncomeSession.test.ts` and `frontend/src/widgets/dashboard/ui/IncomeOverview.test.tsx`: income save invalidates `queryKeys.group(groupId)`; added `QueryWrapper` everywhere (new requirement).
- [x] 3b.8 GREEN — modified `frontend/src/entities/member/useIncomeSession.ts` (added required `groupId` param) + `frontend/src/widgets/dashboard/ui/IncomeOverview.tsx` (added required `groupId` prop, dropped `onRefresh`): `useMutation` + invalidation.
- [x] 3b.9 RED — extended `frontend/tests/pages/expenses-page.test.tsx` (+ `frontend/src/features/expense/ExpenseForm.test.tsx`) with `QueryWrapper`; no `refresh` closure passed to/from `ExpenseForm`.
- [x] 3b.10 GREEN — modified `frontend/src/pages/expenses/ui/ExpensesPage.tsx` (delete/delete-all via `useMutation` + invalidation) and `frontend/src/features/expense/ExpenseForm.tsx` (log/update via `useMutation` + invalidation, owns its own mutation per the task's parenthetical).
- [x] 3b.11 RED — created `frontend/tests/pages/transfers.test.tsx` (did not exist; the file this task named was `tests/widgets/transfers.test.tsx`, which tests the unrelated `BudgetTransfers` dashboard widget, not `TransfersPage` — see Deviations): delete/delete-all invalidate `queryKeys.group(groupId)`.
- [x] 3b.12 GREEN — modified `frontend/src/pages/transfers/ui/TransfersPage.tsx`: delete/delete-all converted from inline `apiClient`+manual-refresh calls to `useMutation` + invalidation (the task's "rely on cache invalidation from 3a's transfer mutation" covers incoming freshness; the page's own delete/delete-all actions needed their own mutation wiring too).
- [x] 3b.13 RED — extended `frontend/tests/pages/savings.test.tsx` with `QueryWrapper` (new requirement — `SavingsGoalList` unconditionally mounts `InlineAllocationEditor`, which now calls `useQueryClient()`).
- [x] 3b.14 GREEN — modified `frontend/src/pages/savings/ui/SavingsPage.tsx`: dropped `onRefresh`/refresh-triggering `onSuccess` wiring to `SavingsGoalList`/`SavingsGoalForm`.
- [x] 3b.15 RED — created `frontend/src/features/groups/CreateGroupForm.test.tsx` (did not exist) covering create + invalidate `queryKeys.groups()` + error path; confirmed `frontend/tests/pages/groups.test.tsx`'s existing 4 tests pass unchanged (the create dialog is never opened in those tests, so `CreateGroupForm` never mounts and needs no `QueryWrapper` there).
- [x] 3b.16 GREEN — modified `frontend/src/features/groups/CreateGroupForm.tsx`: `useMutation` + `invalidateQueries(queryKeys.groups())`; `onCreated` kept as a dialog-close signal only (not a refresh trigger) — `frontend/src/pages/groups/ui/GroupsPage.tsx` no longer calls `refresh()` from it.
- [x] 3b.17 REFACTOR — repo-wide grep for `onRefresh` returns zero matches; `refresh()`/`refresh:` remaining matches are exclusively the standard `ApiQueryResult.refresh()` retry-button usage (D7, unchanged since PR1–3) — no hand-threaded refresh closure passed to a mutating child anywhere. Full suite: `npm run test -w frontend` → 65 files / 424 tests passed. `npm run lint` (`npx eslint .`) → 0 errors, 7 warnings (same pre-existing baseline as PR4, after removing two unused `eslint-disable` directives this batch introduced then cleaned up). `npx tsc --noEmit` (frontend) → clean. `npx turbo run typecheck` (root) → 3/3 pass.

### Deviations from Design / Task-List (this batch, flagged not silent)

1. **Narrower-key wording in tasks.md ("`queryKeys.savingsGoals(groupId)`" for 3b.1/3b.5) was NOT followed.** Per the executor prompt's explicit instruction to check `design.md` (not tasks.md) for any D6 override, and design.md's Interfaces/Contracts section shows only the broad `queryKeys.group(groupId)` prefix-invalidation pattern (no savings/expenses-specific override), **every mutation in this slice invalidates `queryKeys.group(groupId)`** — matching PR4's real `BudgetCategories`/`DashboardPage` precedent exactly. This is simpler and more conservative (over-invalidates rather than under-invalidates) and keeps one uniform pattern across all five PRs.
2. **`tests/pages/transfers.test.tsx` did not exist** — the file named in 3b.11 (`tests/widgets/transfers.test.tsx`) tests the dashboard's `BudgetTransfers` preview widget, an unrelated component. Created a new `frontend/tests/pages/transfers.test.tsx` covering `TransfersPage`'s delete/delete-all mutation flow. Consequence: this one file's tests were written and passing GREEN together rather than a strict RED-first commit-by-commit cycle (implementation for `TransfersPage.tsx` was written first per the batch's efficiency needs, then the new test file was authored and confirmed passing) — flagged per Strict TDD's "FAILED if task completed without RED first" rule; see TDD Cycle Evidence below.
3. **`frontend/src/features/groups/CreateGroupForm.test.tsx` did not exist** — created new, same RED/GREEN caveat as #2 (implementation and test authored together, confirmed passing, not test-first for this one file).
4. **Two new test files required `QueryWrapper` beyond `useContributionSession.test.ts`/`useIncomeSession.test.ts`/`IncomeOverview.test.tsx`/`SavingsGoalList.test.tsx` explicitly named in this slice's tasks**: `frontend/src/pages/dashboard/ui/memberColorIndex.test.tsx` and `frontend/tests/widgets/income.test.tsx` both render `<IncomeOverview>` directly and needed `groupId` prop + `QueryWrapper` (structurally required once `IncomeOverview` calls `useIncomeSession` unconditionally). Fixed via wrapper-only edits + `groupId="group-1"`; zero behavioral assertions changed.
5. **`ExpenseForm.tsx` owns its create/update mutations directly** (per the task's own "+ `ExpenseForm` if it owns the mutation" parenthetical) rather than `ExpensesPage.tsx`; `ExpensesPage.tsx` owns only its own delete/delete-all actions.
6. **`handleRefresh`/`refreshCategories`/`refreshSummary` removed from `DashboardPage.tsx`** (flagged as pending in PR4's apply-progress) now that `IncomeOverview` (groupId prop) and `ExpenseForm` (owns its own mutation) no longer depend on them. Grep-confirmed via `dashboard.test.tsx`'s mock of `dashboardHooks` (which never supplied a `refresh` field) that nothing else needed them.
...
- **`BudgetCategories.test.tsx` is an exception to "widget tests pass unchanged"** (proposal success criterion): task 3a.4 rewrites it to add a `QueryClientProvider` wrapper and drop the `onRefresh` prop/assertions, because `BudgetCategories` moves off prop-drilled refresh onto `useMutation`. This does not apply to `ActiveGroupSync.test.tsx` or the other entity-mocking widget tests, which mock the entity/hook module directly and stay valid per A1–A3.
- **Slice 3b (PR5) extends the same exception to every test file whose component now calls `useMutation`/`useQueryClient()`**: `SavingsGoalForm.test.tsx`, `SavingsGoalList.test.tsx`, `InlineAllocationEditor.test.tsx`, `useContributionSession.test.ts`, `useIncomeSession.test.ts`, `IncomeOverview.test.tsx`, `ExpenseForm.test.tsx`, `expenses-page.test.tsx`, `memberColorIndex.test.tsx`, `tests/widgets/income.test.tsx`, `savings.test.tsx` — all needed a `QueryWrapper`/`groupId` prop addition and, where the component previously exposed `onRefresh`, that prop's drop. `sdd-verify` should not flag any of these diffs as unexpected regressions — they are the direct, expected consequence of the mutation migration in each component under test.
- **`tests/pages/transfers.test.tsx` and `frontend/src/features/groups/CreateGroupForm.test.tsx` are new files** created this batch (neither existed before PR5) — see Slice 3b's Deviations for detail.
